...
def objective(trial, forecast, train_loader, val_loader, test_loader,
              ...
              random_seed, StandardLSTM, Fv1_train_model_LSTM,
    ...
    try:
        ...
        best_val_loss, trained_model, training_history = train_with_hyperparams(
            ...
            batch_size, random_seed, StandardLSTM,
        ...
        # 更新 model_path 包含试验编号
        trial.set_user_attr('model_path', os.path.join(
            model_save_dir, f"trial{trial.number}_{num_layers}layers_{hidden_size}units_lr{learning_rate:.0e}_do{dropout:.2f}",
            f"{model_name_LSTM}_trial{trial.number}_{num_layers}layers_{hidden_size}units_lr{learning_rate:.0e}_do{dropout:.2f}.pth"
        ))
...
def Fv1_hyperparameter_search_optuna_LSTM(num_layers_range, hidden_size_range, learning_rate_range, dropout_range,
                                         ...
                                         StandardLSTM, Fv1_train_model_LSTM, Fv1_evaluate_model_on_test_set_LSTM,
                                         Fv1_evaluate_final_metrics_LSTM, Fv1_save_results_to_txt_LSTM,
    ...
    objective_with_args = lambda trial: objective(
        ...
        lookback, train_ratio, val_ratio, batch_size, random_seed, StandardLSTM,
        Fv1_train_model_LSTM, Fv1_evaluate_model_on_test_set_LSTM,
        Fv1_evaluate_final_metrics_LSTM, Fv1_save_results_to_txt_LSTM,
...
# 修改 save_optuna_results 函数，在保存最佳配置时也记录试验编号
def save_optuna_results(study, model_save_dir, best_trial):
    ...
    with open(best_config_file, 'w', encoding='utf-8') as f:
        ...
        for trial in study.trials:
            if trial.state == optuna.trial.TrialState.COMPLETE:
                ...
                # 添加模型路径信息
                if hasattr(trial, 'user_attrs') and 'model_path' in trial.user_attrs:
                    f.write(f"  模型路径: {trial.user_attrs['model_path']}\n")
    ...
    best_trial_dir = os.path.join(
        model_save_dir,
        f"trial{best_trial.number}_{best_trial.params['num_layers']}layers_{best_trial.params['hidden_size']}units_lr{best_trial.params['learning_rate']:.0e}_do{best_trial.params['dropout']:.2f}"
    )
    os.makedirs(best_trial_dir, exist_ok=True)

    # 复制最佳模型配置到最佳试验文件夹
    best_config_copy = os.path.join(best_trial_dir, "best_trial_config.txt")
    with open(best_config_copy, 'w', encoding='utf-8') as f:
        f.write(f"最佳试验配置:\n")
        f.write(f"试验编号: {best_trial.number}\n")
        f.write(f"验证损失: {best_trial.value:.6f}\n")
        for key, value in best_trial.params.items():
            f.write(f"{key}: {value}\n")


def train_with_hyperparams(trial_number, num_layers, hidden_size, learning_rate, dropout, forecast,
                           train_loader, val_loader, test_loader, criterion,
                           device, max_epochs, patience, model_save_dir,
                           model_name_LSTM, lookback, train_ratio, val_ratio,
                           batch_size, random_seed, StandardLSTM,
    ...
    新增 trial_number 参数，用于在文件夹名中加入试验编号
    """
    # 创建当前超参数对应的保存目录（包含试验编号）
    folder_name = f"trial{trial_number}_{num_layers}layers_{hidden_size}units_lr{learning_rate:.0e}_do{dropout:.2f}"
    current_model_save_dir = os.path.join(model_save_dir, folder_name)
    os.makedirs(current_model_save_dir, exist_ok=True)

    # 根据预测步长选择模型结构
    if forecast == 1:
        model_instance = StandardLSTM(1, hidden_size, num_layers, dropout, 1, 1)
        print(f"试验 #{trial_number}: 使用单步预测模型 - 层数: {num_layers}, 隐藏单元: {hidden_size}, "
              f"学习率: {learning_rate:.6f}, dropout: {dropout:.2f}")
    else:
        model_instance = StandardLSTM(1, hidden_size, num_layers, dropout, 1, forecast)
        print(f"试验 #{trial_number}: 使用多步预测模型，预测步长: {forecast} - 层数: {num_layers}, "
    ...
    # 更新模型名称，包含试验编号
    model_file_name = f"{model_name_LSTM}_trial{trial_number}_{num_layers}layers_{hidden_size}units_lr{learning_rate:.0e}_do{dropout:.2f}"

    # 调用训练函数
    trained_model, training_history = Fv1_train_model_LSTM(
        model_instance, train_loader, val_loader, criterion, optimizer,
        max_epochs, patience, device, current_model_save_dir, model_file_name
    )
    ...
    results_file_path = os.path.join(current_model_save_dir,
                                     f"{model_name_LSTM}_trial{trial_number}_{num_layers}layers_{hidden_size}units_lr{learning_rate:.0e}_do{dropout:.2f}_results.txt")
