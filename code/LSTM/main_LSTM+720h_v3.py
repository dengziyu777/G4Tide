...
from source_code.Fv1p2_preprocess_data_LSTM import Fv1_preprocess_data_LSTM
from source_code.Fv1_StandardLSTM import StandardLSTM,test_model_creation_LSTM
...
from source_code.Fv1_optuna_hyperparameter_tuning_LSTM import Fv1_hyperparameter_search_optuna_LSTM

# 251201 使用实测潮位数据，基于LSTM预报（基准深度学习模型）------------

# %% 1.1. 设置本次训练参数/超参数====================================================================
forecast = 720       # 未来时间步长（72 240 360 720）
lookback = 24       # 历史时间步长（时间单位与batch1_data_file_path中数据一致）
train_ratio = 0.7       # 训练集比例
val_ratio = 0.15        # 验证集比例
batch_size = 2048         # 批次大小（实测4080S可处理30d尺度）
...
model_save_dir = './output_LSTM_+' + str(forecast) + 'h'     # 输出目录
model_name_LSTM = 'LSTM'   # 模型名称
...
criterion = r2_score_loss   # 使用R²作为损失函数
...
# %% 2.2. 预处理2.1中数据，构造数据集=====================================================================
preprocessed_data = Fv1_preprocess_data_LSTM(batch1_data, lookback, forecast, train_ratio, val_ratio)
print(f"2.2. 完成")


# %% 3.0. 测试构造标准LSTM模型---------------------------
# if DEBUG_MODE:
#     model_single, model_multi = test_model_creation_LSTM(hidden_size, num_layers, dropout)  # 仅为测试标准LSTM是否能实现单步、多步预测
# print(f"3.0. 完成")
