...
batch1_data_file_path = '.\A_TwoPoints\Make2024_input\MIKE生成潮位_2024010108toEnd_原数据+8h考虑为北京时间.dat'  # 天文潮数据
batch1_data_scale_factor = 1  # 乘以此数值将其转化为以m为单位
batch1_data_EL_Adjust = 0.0  # 基准面调整值（m）

batch3_data_folder_path = '.\A_TwoPoints\Make2024_input\era5-2024010108toEnd'  # 设置era5等网站下载的环境数据所在文件夹；ERA5中数据为UTC，转北京时间+8；站点顺序需保持一致
...
site_params_file = '.\A_TwoPoints\Make2024_input\模型参数设置-2024010300to123123.dat'
site_params = {}
...
# 本次预测结果输出设置----------------------------------------
output_prediction_folder = f".\A_TwoPoints\Make2024_output\-{use_forward_hours_all}h+0h_{model_type}-P4"  # 便于自动创建输出目录
