...
batch1_data_file_path = '.\A_TwoPoints\Make81to00_input\input_F_MIKE\F_TwoPoints_81to00_+0h.dat'  # 天文潮数据
batch1_data_scale_factor = 1  # 乘以此数值将其转化为以m为单位
batch1_data_EL_Adjust = 0.0  # 基准面调整值（m）

batch3_data_folder_path = '.\A_TwoPoints\Make81to00_input\input_era5'  # 设置era5等网站下载的环境数据所在文件夹；ERA5中数据为UTC，转北京时间+8；站点顺序需保持一致
...
site_params_file = '.\A_TwoPoints\Make81to00_input\O_BHSea_81to00_2site_params.dat'
site_params = {}
...
# 本次预测结果输出设置----------------------------------------
output_prediction_folder = f".\A_TwoPoints\Make81to00_output\-{use_forward_hours_all}h+0h_{model_type}-P4"  # 便于自动创建输出目录
...
if DEBUG_MODE:
    ...
    batch2_data_file_path = '.\A_TwoPoints\Make81to00_input\input_O_BHSea\O_TwoPoints_81to00.dat'  # 设置观测潮位数据所在文件夹（DL训练时，训练其数据覆盖时间）
    ...
    Fv6_three_comparison(batch1_data, predictions, predict_timestamps, batch2_data_adjusted, site_params,
                         ...
                         f"O&BP&MyPaper / {case_name_1st}", f'EL(m)', interval_hours, rotation_user,
